...
G4cout << "t = " << t/s << " s, istep = " << istep << ", nfailed = " << nfailed << G4endl;
    ...
    if(t >= tf || nfailed < 0) break; // done with full tracking step,
                                      // or underflow condition flagged by nfailed < 0
...
{
  // calculate the local B field to use for the Bloch Eq integration.
  // B0_z is constant, B0_x and B0_y are zero unless we add an
  // estimate of the local Earth's field. the Ramsey and dressing fields change
  // depending on time (argument t) and position (argument p).
